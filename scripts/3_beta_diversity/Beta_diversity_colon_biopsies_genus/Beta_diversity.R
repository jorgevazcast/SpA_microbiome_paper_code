...
load(paste0(working_dir, "/1_infiles/Colon_biopsies/physeq.colon.GMPR.genus.RData"))
load(paste0(working_dir, "/1_infiles/Colon_biopsies/physeq.colon.genus.rar.RData"))

in_phylo <-  physeq.colon.genus.rar
otable <- as.matrix( otu_table(in_phylo))
...
Metadata  <- data.frame( sample_data(in_phylo) , stringsAsFactors = F )
...
Varaibles2use <- Varaibles2use[!Varaibles2use %in% c(Variables2eliminate)]
...
out_dir  = "./"
table.ADONIS <- ADONIS_func( in.matrix =  taxa_matrix  , Distance = "bray", in.Metadata = data.frame(Metadata2use) , prefix = paste0(out_dir,"/euclidean") )
