...
#### Read the GLM network 
load("/home/luna.kuleuven.be/u0141268/Dropbox/1-GIANT_last/1-Draft-paper/V1/update_figures/multiomics/Network/glm_network.RData")
Features  <- unique(as.character(c(glm_network$Var1,glm_network$Var2)))

physeq.qmp.motus  <- prune_taxa(taxa_names(physeq.qmp.motus)[taxa_names(physeq.qmp.motus) %in% Features]  , physeq.qmp.motus  )
physeq.qmp.gmm  <- prune_taxa(taxa_names(physeq.qmp.gmm)[taxa_names(physeq.qmp.gmm) %in% Features]  , physeq.qmp.gmm  )
physeq.metabolites.norm  <- prune_taxa(taxa_names(physeq.metabolites.norm)[taxa_names(physeq.metabolites.norm) %in% Features]  , physeq.metabolites.norm  )


#########################################
###### Subset the omic datasets   ##########
Samples2Use <- table(c( sample_names(physeq.qmp.motus)  , sample_names(physeq.qmp.gmm), sample_names(physeq.metabolites.norm)  ))
Samples2Use <- names(Samples2Use[Samples2Use == 3])
...
motus_df <- data.frame(otu_table(physeq.qmp.motus))
gmm_df <- data.frame(otu_table(physeq.qmp.gmm))
metabolites_df <- data.frame(otu_table(physeq.metabolites.norm))
...
#"Disease.SpA"                                                                          
#"Disease_activity.SpAHigh"                                                             
#"Disease_activity.SpALow"                                                              
#"Age_at_visit"                                                                         
#"NSAID_use.NO"                                                                         
#"Smoking.history"                                                                      
Sample_data <- Sample_data[,c("Disease","Disease_activity","Age_at_visit","NSAID_use","Smoking")]
Sample_data <- Sample_data[match( Samples2Use, rownames(Sample_data)),]
...
Linear  <- rbind(Age_at_visit=Age_at_visit,metabolites_df)
Linear <- t(Linear)
colnames(Linear)  <- paste0("Linear.",colnames(Linear))

Boolean  <- data.frame(Disease.SpA=Disease.SpA,Disease_activity,NSAID_use.NO=NSAID_use.NO,Smoking.history=Smoking.history)
colnames(Boolean)  <- paste0("Boolean.",colnames(Boolean))
rownames(Boolean) <- rownames(Sample_data)

Boolean<- data.frame(Boolean[match(Samples2Use, rownames(Boolean)),])
Linear <- data.frame(Linear[match(Samples2Use, rownames(Linear)),])
QMP    <- data.frame(QMP[match(Samples2Use, rownames(QMP)),])
...
retHC <- hc( df2bnlearn, score = "custom", fun = my.bic.GLM, args = list()   )
retBNlearns <- retHC
save(file="retBNlearns.RData",retBNlearns)
plot(retHC)
